...
export default function TeacherDashboard() {
  ...
  // Forms
  const [examForm, setExamForm] = useState({ id: null, title: '', exam_date: '', duration_minutes: '', type: 'MCQ', must_on_camera: true, must_on_microphone: true, course_name: '', course_code: '', university_name: '', max_attempts: 1 });
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col lg:flex-row">
              ...
              <div className="flex justify-between items-center flex-wrap gap-4">
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex bg-gray-100 p-1 rounded-lg">
                    ...
                    >
                      {examFilter === 'live' && <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>}
                      Live
                    </button>
                    <button 
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button 
                    onClick={() => {
                      setExamForm({ id: null, title: '', exam_date: '', duration_minutes: '', type: 'MCQ', must_on_camera: true, must_on_microphone: true, course_name: '', course_code: '', university_name: '', max_attempts: 1 });
                  ...
                  <tbody>
                    {(() => {
                      ...
                      const filteredExams = exams.filter(exam => {
                        const matchesSearch = (exam.title && exam.title.toLowerCase().includes(sq)) ||
                        ...
                        else if (examFilter === 'live') matchesFilter = exam.is_live;
                        else if (examFilter === 'upcoming') matchesFilter = now < examDate;
                        else if (examFilter === 'ended') matchesFilter = now > expireTime;
      ...
      <Modal isOpen={isExamModalOpen} onClose={() => setIsExamModalOpen(false)} title={isEditingExam ? "Edit Exam" : "Schedule Exam"}>
        <form onSubmit={handleSaveExam} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1.5 uppercase">Max Attempts</label>
              <input 
      ...
      <Modal isOpen={isNoQuestionsModalOpen} onClose={() => setIsNoQuestionsModalOpen(false)} title="Action Required">
        <div className="flex flex-col items-center justify-center p-4 text-center">
          <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mb-4">
            <ShieldAlert size={32} />
          </div>
          <h3 className="font-bold text-dark-900 text-lg mb-2">No Questions Set</h3>
          <p className="text-gray-500 text-sm mb-6">
            You cannot make this exam live because it doesn't have any questions. Please go to the <strong>Set Questions</strong> tab to add questions first.
          </p>
          <button 
            onClick={() => {
              setIsNoQuestionsModalOpen(false);
              setActiveTab('set_questions');
