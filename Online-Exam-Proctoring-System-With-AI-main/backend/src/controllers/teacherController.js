...
exports.createExam = async (req, res) => {
  const { title, exam_date, duration_minutes, type, must_on_camera, must_on_microphone, exam_password, course_name, course_code, university_name, max_attempts } = req.body;
  ...
  try {
    const [result] = await db.query(
      'INSERT INTO exams (title, exam_date, duration_minutes, type, teacher_id, must_on_camera, must_on_microphone, exam_password, course_name, course_code, university_name, max_attempts) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [title, exam_date, duration_minutes, type, req.user.id, must_on_camera ?? true, must_on_microphone ?? true, exam_password || null, course_name || null, course_code || null, university_name || null, max_attempts || 1]
    );
...
exports.updateExam = async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      `UPDATE exams SET title=?, exam_date=?, duration_minutes=?, type=?, must_on_camera=?, must_on_microphone=?, exam_password=?, course_name=?, course_code=?, university_name=?, max_attempts=?
       WHERE id=? AND teacher_id=?`,
      [title, exam_date, duration_minutes, type, must_on_camera ?? true, must_on_microphone ?? true, exam_password || null, course_name || null, course_code || null, university_name || null, max_attempts || 1, id, req.user.id]
    );
